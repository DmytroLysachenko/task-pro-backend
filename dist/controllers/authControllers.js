...
const loginUser = async (req, res, next) => {
    try {
        ...
        if (!user.verify) {
            throw HttpError(400, 'User mail is not verified, please check your mail for following instructions');
        }
...
const patchUser = async (req, res, next) => {
    try {
        ...
        if (!subscriptions.includes(subscription)) {
            throw HttpError(400, `Subscription should be one of the following : ${subscriptions.join(', ')}`);
        }
